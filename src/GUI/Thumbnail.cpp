...
vint Thumbnail::_create(const char* const filename) {
    if (this->thumbnail) {
        this->destroy();
        this->status = DX_THUMBNAIL_UNINITIALIZED;
    }

    int channels = 0;
    int _w = 0, _h = 0;
    uint8_t* data = stbi_load(filename, &_w, &_h, &channels, 4);
    if (!data) {
    ...
    if (_h > 512) {
        ...
        // TODO: Why not * channels?
        resized = new uint8_t[fixed_width * fixed_height * 4];
...
inline bool Thumbnail::is_init() const {
    return !this->status;
}
